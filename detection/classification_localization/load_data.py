...
def load_dataset(path, test_split=0.2, val_split=0.2):
    ...
    sets = Datasets()
    train_dataset = Dataset(train_list.values, data_folder)
    validation_dataset = Dataset(validation_list.values, data_folder)
    test_dataset = Dataset(test_list.values, data_folder, test=True)
...
class Dataset(object):
    ...
    def __init__(self, datalist, data_folder, test=False):
...
class DataGenerator(Sequence):
    ...
    def __init__(self, datalist, data_folder, shape, batch_size, onehot=True, preprocess_for=None,  shuffle=True, test=False):
    ...
    def __data_generation(self, filelist_temp):
        ...
        if self._test:
            return X, bboxes, og_images, og_bboxes
        else:
            return X, bboxes
