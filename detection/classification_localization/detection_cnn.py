...
def main(argv):
    ...
    model = build_bbox_model(input_shape)
    model.summary()
    ...
    checkpoint = ModelCheckpoint(output_checkpoint_folder + "model-{epoch:02d}-{val_iou:.7f}.h5",
                                     monitor="val_iou", verbose=1, save_best_only=True,
                                     save_weights_only=False, mode="max", period=1)
    early_stop = EarlyStopping(monitor="val_iou", patience=patience_stop, mode="max")
    reduce_lr = ReduceLROnPlateau(monitor="val_iou", factor=0.2, patience=patience_lr, min_lr=1e-7, verbose=1, mode="max")
    ...
    if not argv.memory:
        ...
        data.train.load_into_memory(shape=input_shape, preprocess_for=preprocess_for)
        data.validation.load_into_memory(shape=input_shape, preprocess_for=preprocess_for)
        print("done")
...
def build_bbox_model2(input_shape):
    detection_net = VGG16(input_shape=input_shape, weights="imagenet", include_top=False)
    for layer in detection_net.layers:
        layer.trainable = False
    
    boxhead = detection_net.output
    size1, size2 = boxhead.shape[1].value, boxhead.shape[2].value
    
    boxhead = Conv2D(filters=4, kernel_size=(size1, size2), name="box_conv2d3")(boxhead)
    boxhead = Reshape((4,), name="box_head")(boxhead)

    model = Model(inputs=detection_net.input, outputs=boxhead)

    optimizer = Adam(lr=0.001)# todo 0.0001

    model.compile(optimizer=optimizer, loss="mean_squared_error", metrics=[iou])
    
    return model

def build_bbox_model(input_shape):
    #detection_net = VGG16(input_shape=input_shape, weights="imagenet", include_top=False)
    detection_net = ResNet50(input_shape=input_shape, weights="imagenet", include_top=False)
    for layer in detection_net.layers:
        layer.trainable = False
    
    boxhead = detection_net.get_layer("activation_49").output
    size1, size2 = boxhead.shape[1].value, boxhead.shape[2].value
    
    boxhead = Conv2D(filters=128, kernel_size=(3,3), padding="same", kernel_regularizer=regularizers.l1(0.1), name="box_conv2d1")(boxhead)
    boxhead = Conv2D(filters=128, kernel_size=(3,3), padding="same", kernel_regularizer=regularizers.l1(0.1), name="box_conv2d2")(boxhead)
    boxhead = Conv2D(filters=4, kernel_size=(boxhead.shape[1].value, boxhead.shape[2].value), kernel_regularizer=regularizers.l1(0.1), name="box_conv2d3")(boxhead)
    boxhead = Reshape((4,), name="box_head")(boxhead)
